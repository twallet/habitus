 ...
 *
 * <pre>
 * QUnit2GS is a Google Apps Script Library that allows Apps Script projects to
be tested
 ...
 *     }
 *
 * - Provide a getResultsFromServer() which simply returns QUnitGS2.getResultsFr
omServer()
 *
 *   For example:
 *
 ...
 *
 *   script.google.com/d/1cmwYQ6H7k6v3xNoFhhcASR8K2_JBJcgJ2W0WFNE8Sy3fAJzfE2Kpbh
_M
 *
 * << Differences from main QUnit library >>
 *
 * - QUnitGS2 provides a wrapper to the main QUnit library. It creates the HTML/
CSS
 *   to display in an Apps Script web app based on the JSON test results
 *   returned by various QUnit callback functions. This is in contrast to how th
e
 ...
 *
 * - The published QUnit library functions are in a separate file for the librar
y
 * creation and API documentation generation to work.
 *
 * - The GUI is not displayed immediately, but rather only after the tests have
run.
 *
 * - Tests are run on Google's servers, not in your browser. The browser only
 * displays test results.
 *
 * - Asynchronous testing is limited. There is no setTimeout() function in Googl
e
  ...
  function push(type, details) {
    ...
    function updateCacheWithTestDetails() {
      ...
      if (type === 'TESTS_RESULTS_ASSERT') {
        ...
        details.expected = typeof details.expected === 'object' ? QUnit.dump.par
se(details.expected) : details.expected
        details.actual = typeof details.actual === 'object' ? QUnit.dump.parse(d
etails.actual) : details.actual
      ...
      function getDiff() {
        ...
        if (typeof details.actual === "number" && typeof details.expected === "n
umber") {
        ...
        } else if (typeof details.actual !== "boolean" && typeof details.expecte
d !== "boolean") {

          diff = QUnit.diff(details.expected, details.actual)

          // don't show diff if there is zero overlap
          showDiff = stripHtml(diff).length !== stripHtml(details.expected).leng
th + stripHtml(details.actual).length;
        }
        ...
        function stripHtml(string) {
          ...
          return string.replace(/<\/?[^>]+(>|$)/g, "").replace(/&quot;/g, "").re
place(/\s+/g, "");
        }
      }
...
/**
 * <pre>
 * This is called by the project using the QUnitGS2 library to get the initial H
TML
 * displayed by the web app.
 *
 * Once the test results are returned by the main QUnit library client-side JS i
n
...
function getHtml() {
  ...
  mainTemplate.title = QUnit.config.title ? QUnit.config.title : "QUnit v2.9.2 f
or Google Apps Script";
  ...
  if (typeof QUnit.config.cssUrl === "string" ) {
    ...
    mainTemplate.styles =
      '<link rel="stylesheet" href="' + QUnit.config.cssUrl + '" type="text/css"
 media="screen" />'

  } else {

    mainTemplate.styles = '<link rel="stylesheet" href="https://code.jquery.com/
qunit/qunit-2.9.2.css">';
  }
...
//  toolbarTemplate.urlConfigs = urlConfigHtml; // TODO
  ...
  htmlCollection.main.testUrl = htmlCollection.toolbar.testUrl = ScriptApp.getSe
rvice().getUrl();
...
/**
 * Not part of the public API, but has to be global so that the web app can see
it
 */
