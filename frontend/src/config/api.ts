...
const getEnvValue = (): string => {
  ...
  const globalImport = (globalThis as { import?: { meta?: { env?: { VITE_API_BASE_URL?: string } } } }).import;
  ...
  try {
    const viteEnv = new Function('return import.meta?.env?.VITE_API_BASE_URL')();
    if (viteEnv) {
      return viteEnv;
    }
  } catch {
  ...
  return (typeof process !== 'undefined' && process.env?.VITE_API_BASE_URL) || 'http://localhost:3001';
};
